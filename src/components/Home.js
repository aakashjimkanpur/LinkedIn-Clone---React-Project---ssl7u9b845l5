...
const Home = () => {
  ...
  let userName = usercontext.name.userName;
  ...
  function post() {
    if (!postData) toast.error("Please Enter Some Text");
      ...
      const oldPost = window.localStorage.getItem("postList");
      let combinedPost = [];
      if (!oldPost) window.localStorage.setItem("postList", JSON.stringify([]));
      else combinedPost = JSON.parse(oldPost);
      const valueToStore = {
        postBy: userName,
