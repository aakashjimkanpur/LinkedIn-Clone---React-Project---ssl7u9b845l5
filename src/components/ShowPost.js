...
const ShowPost = (prop) => {
  ...
  let postId = prop.postDetail._id;
  let postLikeList = prop.postDetail.likeList;
  let postCommentList = prop.postDetail.commentList;
  let userProfileUrl =
  ...
  useEffect(() => {
    const idx = postLikeList.findIndex((i) => {
      return i === userId;
    });
  ...
  function likeClick(id) {
    fetch(`${baseURL}/post/toggleLike`, {
      ...
      .then((data) => data.json())
      .then((data) => {
        data.isLiked ? setIsLiked(true) : setIsLiked(false);
      });
  }
  return (
    <div>
      ...
      >
        <div className="d-flex">
          <div>Post By </div>
          <div> : </div>
          <div className="fw-bold">{prop.postDetail.postByName}</div>
        </div>
        <hr className="mt-0 " />
        <div
          style={{
